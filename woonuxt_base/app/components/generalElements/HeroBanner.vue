<template>
  <div class="relative mx-auto">
    ...
    <div class="container absolute inset-0 flex flex-col items-start justify-center bg-gradient-to-l from-gray-200 md:bg-none">
      <h1 class="text-3xl font-bold md:mb-4 md:text-4xl lg:text-6xl">{{ $t('hero.title') }}</h1>
      <h2 class="text-lg font-bold md:mb-4 lg:text-3xl">{{ $t('hero.subTitle') }}</h2>
      <div class="max-w-sm mb-8 text-md font-light lg:max-w-md text-balance">
        <p>{{ $t('hero.description') }}</p>
      </div>
      <NuxtLink class="px-6 py-3 font-bold text-white bg-gray-800 rounded-xl hover:bg-gray-800" to="/products">{{ $t('hero.cta') }}</NuxtLink>
    </div>
  </div>
</template>
